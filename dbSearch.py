...
class Listing:
    def __init__(self, name, price, drink_type, count, volume, alcPerc, category, rating, sku, value, adjValue, sale,
                 image):
        self.name = name
        self.price = price
...
def process_search(maxPrice=0, drink_type="all", filterStores=[], only_open_stores=True, response_url=None,
    ...
    for listing in listings:
        ...
        card['text']['text'] = card['text']['text'] \
            ...
            .replace('{price}',
                     str(round((listing.price * 1.15 + (0.1 * listing.count if listing.count > 1 else 0.2)), 2))) \
    ...
    try:
        ...
        res = req.post(response_url, headers={'Authorization': BOT_TOKEN, 'Content-type': 'application/json'},
                       json={"text": user_return_modal['text'], "blocks": user_return_modal['blocks']})
        print(res.text)
